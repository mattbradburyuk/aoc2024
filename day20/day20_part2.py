...
def check_for_shortcuts(grid, pos, max_len):

    for offset_r in range(-max_len, + max_len + 1):
        max_c = max_len - abs(offset_r)
        min_c = - max_c
        for offset_c in range(min_c, max_c + 1):
            sc_len = abs(offset_r) + abs(offset_c)
            # print(f"pos: {pos}, offset_r: {offset_r}, offset_c: {offset_c}, sc_len: {sc_len}")
            np = (pos[0] + offset_r, pos[1] + offset_c)
            if in_bounds(grid, np) and isinstance(grid_value(grid, np),int):
                short_cut = grid_value(grid, np) - grid_value(grid, pos) - sc_len
                if (short_cut > 0):
                    cheats.append(short_cut)
...
for ind_row, row in enumerate(grid):
    for ind_col, tile in enumerate(row):
        if isinstance(tile, int):
            check_for_shortcuts(grid, (ind_row, ind_col), max_len)   
